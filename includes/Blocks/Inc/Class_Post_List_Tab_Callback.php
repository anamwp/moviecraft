...
namespace MovieCraft\Blocks\Inc;
...
class Class_Post_List_Tab_Callback
{
	...
	{
		add_action('wp_ajax_nopriv_handle_category_post_content', array($this, 'moviecrafthandle_category_post_content'));
		add_action('wp_ajax_handle_category_post_content', array($this, 'moviecrafthandle_category_post_content'));
	}
	/**
	...
	public function gs_handle_category_post_content()
	{
		...
		if (empty($gs_ajax_nonce) || ! wp_verify_nonce($gs_ajax_nonce, 'moviecraftajax_nonce')) {
...
?>
		...
		<?php else : ?>
			<div class="post card shadow-md hover:shadow-lg rounded border-solid border-black-200 border-x border-y p-8">
				<h2 class="mt-4 inline-block font-poppins text-xl text-slate-900 hover:text-slate-600	transition font-medium"><?php echo esc_html('No Posts Found', 'moviecraft'); ?></h2>
			</div>
		<?php endif; ?>
<?php
		die();
